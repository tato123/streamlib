...
- Twenty-four Python processors take keyword configuration: six engine-tree fixtures
  ...
  (`testing.py:106`).
- The four casts (`video_frame.rs:22-54`, `audio_block.rs:27-54`,
  `encoded_video_frame.rs:71-101`, `encoded_audio_packet.rs:111-146`) derive serde only.
  Two carry `#[serde(with = "serde_bytes")]` bytes and one a hand-written bin module
  (`encoded_audio_packet.rs:40-71`); their nested colour and codec enums carry
  `#[serde(rename = …)]`.
- The control plane is mounted by two hosts through
  `ApiServerControlPlaneHostConfig { bind_host, bind_port, node_name }`
  (`runtime/streamlib-api-server/src/control_plane_host.rs:13-54`): the wheel
  (`sdk/streamlib-python-wheel/src/python_control_plane_hosting.rs:21-40`) and the rig
  example (`runtime/streamlib-engine/examples/codec_roundtrip_rig.rs:1017-1024`).
  `AppState` holds the runtime handle and the OpenAPI document (`state.rs:12-16`);
...
- **The four casts derive `JsonSchema`** with their nested types — `ColorInfo`,
  ...
  `#[schemars(with = "MsgpackBinaryBytes")]`, a media-crate helper whose schema is
  `{"type": "string", "contentEncoding": "binary"}` with a description saying the wire
  carries a msgpack `bin`. Renames and `skip_serializing_if` are honoured by the derive,
  so the documented keys are the wire keys the existing tests pin
  (`video_frame.rs::video_frame_bag_carries_the_documented_keys`).
- **The media crate exports the set.** `BagConventionDescriptor { name: String, schema:
...
- **The host hands them in.** `ApiServerControlPlaneHostConfig` gains
  ...
  media crate's set (`python_control_plane_hosting.rs:21-40`,
  `codec_roundtrip_rig.rs:1017-1024`). The api-server's `Cargo.toml` still names no media
  crate; the two-spellings rule is kept by serving the same documents at
  `GET /api/bag-conventions` beside `/api/registry`.
...
- **Binary fields render as `contentEncoding: binary`.** JSON Schema has no bytes type;
  this is the 2019-09+ vocabulary word for it, and the description names msgpack `bin`.
